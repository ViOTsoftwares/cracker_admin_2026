...
export default function List() {
        ...
        cell: ({ row }) => (
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => {
                navigate.push("/testimonial/update-testimonial/" + row?.original?._id);
              }}
              className="
  ...
  const fetchData = async ({
    ...
    const res = await GetTestimoalApi(body);
    ...
    try {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          const reponse: any = await DeleteTestimoalApi({ id });
          if (reponse.success) {
...
<button
          onClick={() => {
            navigate.push("/testimonial/add-testimonial");
          }}
          className="
