...
import { OneCategoryApi, UpdateCategoryApi } from "@/Api/category";
...
const UpdateCategoryPage = () => {
  ...
  const GetOneCategory = async () => {
    try {
      const response = await OneCategoryApi(id);
      if (response?.success) {
        setFormValues({
        ...
        setImagePreview(response.result?.image || null);
      } else {
        toastMessage("Failed to fetch category details", "error");
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await UpdateCategoryApi(fs);
      ...
      <div className="bg-white rounded-2xl shadow-sm border mt-4">
        <div className="border-b px-8 py-6">
          ...
          <p className="text-sm text-gray-500 mt-1">
            Update category classification information
          </p>
        </div>
